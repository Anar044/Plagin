...
namespace Resto.Front.Api.HorecaControlPlugin.Core.Application.Services
{
    ...
    public class ReportGenerator : IReportGenerator
    {
        ...
        public IPluginToServer GenerateSummaryReport(List<IOrder> orders, int activeEmployees)
        {
            if (orders == null)
                throw new ArgumentNullException(nameof(orders));

            PluginContext.Log.Debug($"GenerateSummaryReport :: started.");
            ...
            try
            {
                var groupedOrders = orders
                    ...
                    .ToDictionary(
                        ...
                        tg => tg.GroupBy(o => o.Tables.First().RestaurantSection)
                ...
                data.CalculateTotal();
                PluginContext.Log.Debug("GenerateSummaryReport completed.");
            }
            catch (Exception ex)
            {
                if (Properties.Settings.Default.Debug)
                    PluginContext.Log.Error($"GenerateSummaryReport :: {ex.Message}", ex);
                else
                    PluginContext.Log.Error($"GenerateSummaryReport :: {ex.Message}");
            }

            return data;
        }

        public IPluginToServer GenerateRevenueByFloorsReport(List<IOrder> orders, int activeEmployees)
        {
            if (orders == null)
                throw new ArgumentNullException(nameof(orders));

            PluginContext.Log.Debug($"GenerateRevenueByFloorsReport :: started.");
            var data = new PluginToServerSummaryOfRestaurant
            {
                ActiveEmployees = activeEmployees,
            };
            ...
            try
            {
                foreach (var tgGroup in orders.GroupBy(x => x.Tables.FirstOrDefault()?.RestaurantSection?.TerminalsGroup))
                {
                    ...
                    PluginContext.Log.Debug(
                        $"GenerateRevenueByFloorsReport :: tgGroup {tgGroup.Key.Name} started");
                    foreach (var rsGroup in tgGroup.GroupBy(x => x.Tables.FirstOrDefault()?.RestaurantSection))
                    {
                        ...
                        if (orderRestuarantSections is null)
                            continue;
                        PluginContext.Log.Debug(
                            $"GenerateRevenueByFloorsReport :: rsGroup {rsGroup.Key.Name} started");
                        ...
                        PluginContext.Log.Debug(
                            $"GenerateRevenueByFloorsReport :: rsGroup {rsGroup.Key.Name} finished");
                    }

                    PluginContext.Log.Info(orderTerminalGroup.ToJson());
                    PluginContext.Log.Debug(
                        $"GenerateRevenueByFloorsReport :: tgGroup {tgGroup.Key.Name} finished.");
                }

                data.TerminalsGroups.ForEach(x => x.CalculateFloorTotalGroup());
                data.CalculateTotal();
                PluginContext.Log.Debug($"GenerateRevenueByFloorsReport :: finished.");
            }
            catch (Exception ex)
            {
                if (Properties.Settings.Default.Debug)
                    PluginContext.Log.Error($"GenerateRevenueByFloorsReport :: {ex.Message}", ex);
                else
                    PluginContext.Log.Error($"GenerateRevenueByFloorsReport :: {ex.Message}");
            }

            return data;
        }

        public IPluginToServer GenerateWaitersReport(List<IOrder> orders)
        {
            if (orders == null)
                throw new ArgumentNullException(nameof(orders));
                    ...
                    foreach (var waiterOrders in orders.Where(o => o.Waiter != null).GroupBy(x => x.Waiter))
                    {
                        ...
                        if (waiterOrdersInTg is null)
                        {
                            waiterOrdersInTg = new PluginToServerRevenueByWaitersTerminalGroupWaiter
                            {
                                ...
                                HighRiskOperations = _repository.GetHighRiskOperationsCount(waiterOrders.Key),
                                OpenedOrders = 0,
                                ClosedOrders = 0,
        ...
        public IPluginToServer GenerateCurrentShiftOrders(List<IOrder> orders, List<IReserve> reserves)
        {
            if (orders == null)
                throw new ArgumentNullException(nameof(orders));
            if (reserves == null)
            ...
            try
            {
                var shift = _repository.GetOpenShift();
                var shiftOpenTime = shift?.OpenTime ?? DateTime.MinValue;

                var reservesWithOrders = reserves.Where(x =>
                        x.Order != null
                        && x.CancelReason == null
                        && x.EstimatedStartTime >= shiftOpenTime)
        ...
        public IPluginToServer GenerateTopTenMealsReport(List<IOrder> orders)
        {
            if (orders == null)
                throw new ArgumentNullException(nameof(orders));
            ...
            {
                PluginContext.Log.Error($"GenerateTopTenMealsReport :: {ex.Message}");
            }

            return result;
        }

        public IPluginToServer GenerateStopListReport(Dictionary<IProduct, decimal> products)
        {
            if (products == null)
                throw new ArgumentNullException(nameof(products));

            PluginToServerStopListRemainingMeals result = null;
            PluginContext.Log.Debug($"GenerateStopListReport :: started");
            try
            {
                ...
                PluginContext.Log.Debug($"GenerateStopListReport :: finished");
            }
            catch (Exception ex)
            {
                if (Properties.Settings.Default.Debug)
                    PluginContext.Log.Error($"GenerateStopListReport :: {ex.Message}", ex);
                else
                    PluginContext.Log.Error($"GenerateStopListReport :: {ex.Message}");
            }

            return result;
        }

        public IPluginToServer GenerateTablesWithOpenOrders(List<IOrder> orders)
        {
            if (orders == null)
                throw new ArgumentNullException(nameof(orders));
        ...
        public IPluginToServer GenerateHighRiskOperationsReport()
        {
            PluginToServerHighRiskOperation result = null;
            try
            {
                PluginContext.Log.Debug($"GenerateHighRiskOperationsReport :: started");

                var hr = _repository.GetAllHighRiskOperations();
                if (hr.Any())
                {
                ...
                PluginContext.Log.Debug($"GenerateHighRiskOperationsReport :: finished");
            }
            catch (Exception ex)
            {
                if (Properties.Settings.Default.Debug)
                    PluginContext.Log.Error($"GenerateHighRiskOperationsReport :: {ex.Message}", ex);
                else
                    PluginContext.Log.Error($"GenerateHighRiskOperationsReport :: {ex.Message}");
            }

            return result;
        }

        public IPluginToServer GenerateOrderDetails(int orderNumber, List<IOrder> orders, List<IReserve> reserves)
        {
            if (orders == null)
                throw new ArgumentNullException(nameof(orders));
            if (reserves == null)
            ...
            {
                if (Properties.Settings.Default.Debug)
                    PluginContext.Log.Error($"GenerateOrderDetails :: {ex.Message}", ex);
                else
                    PluginContext.Log.Error($"GenerateOrderDetails :: {ex.Message}");
            }
